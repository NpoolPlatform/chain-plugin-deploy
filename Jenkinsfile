pipeline {
  ...
  stages {
    ...
    stage('Preparing') {
      steps {
          sh 'cd .easy-deploy; ansible-playbook playbooks/prepare/prepare.yml -i inventories/$CUSTOMER/plugin/$ENVIRONMENT'
      }
    }

    stage('Deploying') {
      steps {
          sh 'cd .easy-deploy; ansible-playbook playbooks/$CUSTOMER/wallet.yml -i inventories/$CUSTOMER/plugin/$ENVIRONMENT'
      }
    }
  }
